...
//!
//! This is the canonical app boundary for new shells: one opaque handle,
//! fire-and-forget actions, and bounded snapshots. The existing
//! [`HighlighterCore`](crate::client::HighlighterCore) is retained behind this
//! facade as a compatibility object while platform screens migrate.
...
use crate::models::{
    ArticleRecord, ArtifactPreview, BookmarkSetRecord, CommunitySummary, CurrentUser,
    HighlightDraft, HighlightRecord, HydratedHighlight, NostrConnectOptions, ProfileMetadata,
    ReadingFeedItem, RelayStatus, RoomRecommendation, WebBookmarkRecord,
};
...
pub enum HighlighterHomeFeedItem {
    Highlights {
        stable_id: String,
        sort_key: u64,
...
pub trait HighlighterAppReconciler: Send + Sync {
    fn on_update(&self, update: HighlighterAppUpdate);
}
...
pub struct HighlighterNmpApp {
    ...
    legacy_callback: Arc<RwLock<Option<Arc<dyn EventCallback>>>>,
...
impl HighlighterNmpApp {
    ...
    pub fn new(config: HighlighterAppConfig) -> Arc<Self> {
        ...
        let legacy_callback = Arc::new(RwLock::new(None));

        core.set_event_callback(Arc::new(CoreDeltaMultiplexer {
            tx: tx.clone(),
            legacy_callback: legacy_callback.clone(),
        }));
    ...
    /// Compatibility handle for screens that have not yet moved to
    /// `dispatch` + `state`. New code should prefer the NMP facade.
    pub fn legacy_core(&self) -> Arc<HighlighterCore> {
        self.core.clone()
    }
    ...
    pub fn dispatch(&self, action: HighlighterAppAction) {
        if let Err(err) = self.tx.try_send(KernelMsg::Action(action)) {
    ...
    pub fn listen_for_updates(&self, reconciler: Arc<dyn HighlighterAppReconciler>) {
        *self.reconciler.write() = Some(reconciler.clone());
        reconciler.on_update(HighlighterAppUpdate::FullState {
            state: self.state(),
        });
    }

    pub fn set_legacy_event_callback(&self, callback: Arc<dyn EventCallback>) {
        *self.legacy_callback.write() = Some(callback.clone());
    ...
    pub fn clear_legacy_event_callback(&self) {
        *self.legacy_callback.write() = None;
    }
}
...
impl EventCallback for CoreDeltaMultiplexer {
    fn on_data_changed(&self, delta: Delta) {
        if let Some(callback) = self.legacy_callback.read().clone() {
            callback.on_data_changed(delta.clone());
        }
        if let Err(err) = self.tx.try_send(KernelMsg::CoreDelta(delta)) {
            log_send_failure(err);
        }
    }
}
...
fn spawn_actor(
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let mut pending_isbn_lookups = BTreeSet::<String>::new();
            let mut pending_web_metadata = BTreeSet::<String>::new();
            let mut profile_handles = BTreeMap::<String, u64>::new();
            let mut profile_pubkeys_by_handle = BTreeMap::<u64, String>::new();
            let mut app_scope_subscriptions = AppScopeSubscriptions::default();
            let mut search_runtime = SearchRuntime::default();
            let mut create_account_runtime = CreateAccountRuntime::default();
            let mut home_feed_runtime = HomeFeedRuntime::default();
            let mut bookmark_runtime = BookmarkRuntime::default();
            let mut profile_view_runtime = ProfileViewRuntime::default();
            let mut article_reader_runtime = ArticleReaderRuntime::default();
            let mut room_explorer_runtime = RoomExplorerRuntime::default();
                ...
                match msg {
                    KernelMsg::Action(action) => {
                        ...
                        handle_action(
                            ...
                            &reconciler,
                            &mut pending_joins,
                            &mut pending_isbn_lookups,
                            &mut pending_web_metadata,
                            &mut profile_handles,
                            &mut profile_pubkeys_by_handle,
                            &mut app_scope_subscriptions,
                            &mut search_runtime,
                            &mut create_account_runtime,
                            &mut home_feed_runtime,
                            &mut bookmark_runtime,
                            &mut profile_view_runtime,
                            &mut article_reader_runtime,
                            &mut room_explorer_runtime,
                            &tx,
                            visible_limit,
                            &local_state_path,
                        );
                    ...
                    KernelMsg::CoreDelta(delta) => {
                        runtime.block_on(handle_core_delta(
                            delta,
                            &core,
                            &state,
                            &reconciler,
                    ...
                    KernelMsg::Stop => {
                        clear_app_scope_subscriptions(&core, &mut app_scope_subscriptions);
                        clear_profile_subscriptions(
                            &core,
                            &mut profile_handles,
                            &mut profile_pubkeys_by_handle,
                        );
                        clear_search_runtime(&core, &mut search_runtime);
                        clear_home_feed_runtime(&core, &mut home_feed_runtime);
                        clear_bookmark_runtime(&core, &mut bookmark_runtime);
                        clear_profile_view_runtime(&core, &mut profile_view_runtime);
                        clear_article_reader_runtime(&core, &mut article_reader_runtime);
                        break;
                    }
                }
            }
...
fn handle_action(
    ...
    app_scope_subscriptions: &mut AppScopeSubscriptions,
    search_runtime: &mut SearchRuntime,
    create_account_runtime: &mut CreateAccountRuntime,
    home_feed_runtime: &mut HomeFeedRuntime,
    bookmark_runtime: &mut BookmarkRuntime,
    profile_view_runtime: &mut ProfileViewRuntime,
    article_reader_runtime: &mut ArticleReaderRuntime,
    room_explorer_runtime: &mut RoomExplorerRuntime,
    ...
    match action {
        ...
        HighlighterAppAction::SignInNsec {
            ...
            match core.login_nsec(nsec.clone()) {
                Ok(_) => {
                    ...
                    if persist {
                        emit_update(
                            state,
                            reconciler,
                            HighlighterAppUpdate::PersistSessionCredential {
                                credential: HighlighterSessionCredential::Nsec { nsec },
                            },
                        );
                    }
                }
                Err(err) => {
                    ...
                    if clear_stored_on_failure {
                        emit_update(
                            state,
                            reconciler,
                            HighlighterAppUpdate::ClearSessionCredentials,
                        );
                    }
                }
            }
        ...
        HighlighterAppAction::PairBunker {
            ...
            match runtime.block_on(core.pair_bunker(uri.clone())) {
                Ok(_) => {
                    set_toast(state, None);
                    runtime.block_on(hydrate_app_chrome(
                        core,
                    ...
                    if persist {
                        emit_update(
                            ...
                            reconciler,
                            HighlighterAppUpdate::PersistSessionCredential {
                                credential: HighlighterSessionCredential::BunkerUri { uri },
                            },
                        );
                    }
                }
            ...
            match runtime.block_on(core.start_nostr_connect(options)) {
                Ok(uri) => {
                    ...
                    emit_update(
                        state,
                        reconciler,
                        HighlighterAppUpdate::OpenExternalUrl {
                            url: append_callback_url(uri, &callback_url),
                        },
                    );
                }
                Err(err) => {
        ...
        HighlighterAppAction::SearchOpened => {
            ...
            let changed = start_search_worker_if_idle(core, state, search_runtime, actor_tx);
            if changed {
                emit(state, reconciler);
            } else {
                emit(state, reconciler);
        ...
        HighlighterAppAction::CompleteOnboarding => {
            runtime.block_on(complete_onboarding(core, state, local_state_path));
            emit(state, reconciler);
        }
...
async fn handle_core_delta(
    ...
    match delta.change {
        DataChangeType::SignerConnected { user } => {
            state.write().chrome.current_user = Some(user);
            hydrate_app_chrome(core, state, pending_joins, visible_limit).await;
            ensure_signed_in_app_scope(core, state, app_scope_subscriptions).await;
...
fn build_home_feed_items(
    ...
    items.reserve(group_order.len().saturating_add(reads.len()));
    ...
    for key in group_order {
        ...
        items.push(HighlighterHomeFeedItem::Highlights {
    ...
    for read in reads {
        ...
        items.push(HighlighterHomeFeedItem::Read {
    ...
    items.sort_by(|left, right| home_feed_sort_key(right).cmp(&home_feed_sort_key(left)));
    items
}
...
fn home_feed_sort_key(item: &HighlighterHomeFeedItem) -> u64 {
    match item {
        HighlighterHomeFeedItem::Highlights { sort_key, .. }
        | HighlighterHomeFeedItem::Read { sort_key, .. } => *sort_key,
    }
}
...
async fn ensure_signed_in_app_scope(
    ...
    {
        match core.init_default_blossom_servers().await {
            Ok(()) => subscriptions.initialized_blossom_defaults_for_pubkey = Some(user_pubkey),
            Err(err) => set_toast(
                state,
                Some(HighlighterToast {
                    kind: HighlighterToastKind::Error,
...
fn start_create_account_request(
    ...
    match thread::Builder::new()
        ...
        .spawn(move || {
            ...
            if actor_tx
                .send(KernelMsg::AccountCreateResolved { generation, result })
                .is_err()
            {
                tracing::warn!("highlighter NMP actor is stopped");
            }
...
fn handle_account_create_resolved(
    ...
    match result {
        Ok(outcome) => {
            tokio_runtime.block_on(hydrate_app_chrome(
                core,
                state,
                pending_joins,
                visible_limit,
            ));
            tokio_runtime.block_on(ensure_signed_in_app_scope(
                core,
                state,
                app_scope_subscriptions,
            ));
            if home_feed_runtime.is_open {
                tokio_runtime.block_on(refresh_home_feed(core, state, home_feed_runtime));
            }
            {
                let mut current = state.write();
                current.create_account.is_creating = false;
            ...
            emit_update(
                ...
                reconciler,
                HighlighterAppUpdate::PersistSessionCredential {
                    credential: HighlighterSessionCredential::Nsec { nsec: outcome.nsec },
                },
            );
        }
        Err(message) => {
...
fn sort_articles_newest_first(articles: &mut [ArticleRecord]) {
    articles.sort_by(|a, b| article_sort_key(b).cmp(&article_sort_key(a)));
}
...
async fn complete_onboarding(
    ...
    set_onboarding_finishing(state, true);
    ...
    current.toast = Some(HighlighterToast {
        kind: if failures == 0 {
            HighlighterToastKind::Success
        } else {
            HighlighterToastKind::Info
        },
        message: if failures == 0 {
            "Welcome to Highlighter".into()
        } else {
            "Saved your interests".into()
        },
    });
    current.bump();
}

fn set_onboarding_finishing(state: &Arc<RwLock<HighlighterAppState>>, is_finishing: bool) {
    let selected = {
        let current = state.read();
        selected_onboarding_ids(&current.onboarding)
    };
    let mut current = state.write();
    let is_complete = current.onboarding.is_complete;
    current.onboarding = onboarding_snapshot(is_complete, selected, is_finishing);
    current.bump();
}
...
fn emit(
    ...
    emit_update(
        state,
        reconciler,
        HighlighterAppUpdate::FullState {
...
fn emit_update(
    _state: &Arc<RwLock<HighlighterAppState>>,
    reconciler: &Arc<RwLock<Option<Arc<dyn HighlighterAppReconciler>>>>,
    update: HighlighterAppUpdate,
) {
    if let Some(reconciler) = reconciler.read().clone() {
        reconciler.on_update(update);
    }
}
...
mod tests {
    ...
    struct TestReconciler {
        tx: std::sync::mpsc::Sender<HighlighterAppUpdate>,
    }
    ...
    fn next_update(rx: &std::sync::mpsc::Receiver<HighlighterAppUpdate>) -> HighlighterAppUpdate {
        rx.recv_timeout(Duration::from_secs(5))
            .expect("app update within timeout")
    }

    fn next_state(rx: &std::sync::mpsc::Receiver<HighlighterAppUpdate>) -> HighlighterAppState {
        for _ in 0..16 {
            if let HighlighterAppUpdate::FullState { state } = next_update(rx) {
                return state;
            }
        }
    ...
    fn valid_sign_in_emits_secret_storage_side_effect_after_validation() {
        ...
        for _ in 0..16 {
            match next_update(&rx) {
                HighlighterAppUpdate::FullState { state } => {
                    saw_signed_in |= state.chrome.current_user.is_some();
                }
                HighlighterAppUpdate::PersistSessionCredential { credential } => {
    ...
    fn invalid_sign_in_surfaces_error_as_state() {
        ...
        for _ in 0..4 {
            match rx.recv_timeout(Duration::from_secs(5)) {
                Ok(HighlighterAppUpdate::FullState { state }) => {
                    saw_error = state
                        .toast
                        .as_ref()
                ...
                Ok(HighlighterAppUpdate::ClearSessionCredentials) => saw_clear = true,
                Ok(HighlighterAppUpdate::PersistSessionCredential { .. }) => {}
                Ok(HighlighterAppUpdate::OpenExternalUrl { .. }) => {}
    ...
    fn home_feed_merge_groups_dedupes_sorts_and_bounds_in_rust() {
        ...
        assert!(matches!(
            &items[0],
            HighlighterHomeFeedItem::Highlights { stable_id, sort_key, .. }
                if stable_id == "h:evt:solo" && *sort_key == 20
        ));
        assert!(matches!(
            &items[1],
            HighlighterHomeFeedItem::Read { stable_id, sort_key, .. }
                if stable_id == "r:30023:author:article-b" && *sort_key == 10
        ));
        match &items[2] {
            HighlighterHomeFeedItem::Highlights {
                stable_id,
                sort_key,
                highlights,
                highlight_count,
            } => {
                assert_eq!(stable_id, "h:src:30023:author:article-a");
                assert_eq!(*sort_key, 8);
                assert_eq!(*highlight_count, 8);
                assert_eq!(highlights.len(), HOME_FEED_GROUP_HIGHLIGHT_LIMIT);
                assert_eq!(highlights[0].highlight.event_id, "highlight-0");
            }
            _ => panic!("expected grouped highlights"),
        }
    }

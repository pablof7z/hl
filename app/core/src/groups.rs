...
// #21: KIND_CREATE_INVITE / MAX_CODES_PER_INVITE_EVENT / INVITE_CODE_ALPHABET
// moved to `kernel::domains::share` alongside the kernel-owned invite mint.
...
// #21: bespoke `create_invite_codes` (kind:9009 code-gen + publish) DELETED —
// the kernel `hl.share.mint_invite` action now mints codes (kernel-owned RNG)
// and publishes via the field-complete nmp `nmp.nip29.create_invite` path
// (`room_home::reduce_action_create_room_invites`). Code generation moved to
// `kernel::domains::share::generate_invite_code`.

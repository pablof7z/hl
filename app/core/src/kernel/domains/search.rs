...
mod tests {
    ...
    //   rust-club  : public+open, name "Rust Club", about "We love Rust"  → matches "rust"
    //   closed-rust: public+closed                                         → excluded
    //   private-rust: private+open                                         → excluded
    //   python-world: public+open, name "Python World"                     → no match on "rust"
    //
    // The kernel scan receives equivalent DiscoveredRow inputs.
    // Dedup: joined row for "rust-club" (member_count=1) is overwritten by
    // discovered row (member_count=42). We assert the same group_ids in the
    // same order as the expected algorithm output.
    #[test]
    fn parity_community_scan_matches_bespoke_algorithm() {
        let relay = "wss://relay.test";

        let communities_joined = vec![
            // Joined row for rust-club (will be overwritten by discovered below)
            crate::kernel::snapshot::CommunityRow {
        ...
        let discovered_groups = vec![
            ...
            crate::kernel::snapshot::DiscoveredRow {
                ...
                host_relay_url: relay.to_string(),
                name: Some("Closed Rust".to_string()),
            ...
            // excluded (no "rust" in name or about)
            crate::kernel::snapshot::DiscoveredRow {
        ...
        // Expected: only "rust-club" passes all filters.
        assert_eq!(
            results.len(),
            1,

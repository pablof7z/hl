...
final class PodcastPlayerStore {
    ...
    func load(artifact: ArtifactRecord) {
        let urlString = [artifact.preview.audioUrl, artifact.preview.audioPreviewUrl]
        ...
        waveformPeaks = []
        waveformTask?.cancel()
        let dur = Double(artifact.preview.durationSeconds ?? 0)
    ...
    func play() {
        // Cold-launch case: MiniPlayer was rehydrated but the kernel has not
        // loaded the player. Route through `load` to dispatch audioPlay.

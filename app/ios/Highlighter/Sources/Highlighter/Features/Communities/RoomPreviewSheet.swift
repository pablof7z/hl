...
struct RoomPreviewSheet: View {
    ...
    private var actionProjection: RoomPreviewActionProjection {
        ...
        let alreadyJoined = appStore.joinedCommunities.contains(where: {
            $0.id.trimmingCharacters(in: .whitespaces) == roomId
        })
    ...
    private var headerProjection: RoomPreviewHeaderProjection {
        ...
        let memberCountLabel: String? = room.memberCount.flatMap { count in
    ...
    private var insideSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let store = roomStore, !store.artifacts.isEmpty {
                ...
                let lastIndex = visible.isEmpty ? 0 : visible.count - 1
                let rows: [RoomPreviewArtifactRowProjection] = visible.enumerated().map { index, artifact in
                    ...
                    let subtitle: String? = {
                        ...
                        if !artifact.preview.domain.isEmpty { return artifact.preview.domain }
                        return nil
                    }()

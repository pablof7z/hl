...
struct NostrSelectableText: UIViewRepresentable {
    ...
    /// Optional "Highlight with note" action. When non-nil, a second
    /// "Highlight with note" action is added to the selection edit menu.
    let onSelectWithNote: ((String, String) -> Void)?
    ...
    init(
        ...
        onSelectWithNote: ((String, String) -> Void)? = nil,
        onLink: @escaping (URL) -> Void
    ) {
        self.attributed = attributed
        self.onSelect = onSelect
        self.onSelectWithNote = onSelectWithNote
        self.onLink = onLink
    }
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        // Inject a "Highlight" action (and optionally "Highlight with note")
        // into the selection edit menu (iOS 16+).
        func textView(
            _ textView: UITextView,
            ...
            var actions: [UIMenuElement] = [highlight]
            if let onNote = parent.onSelectWithNote {
                let noteAction = UIAction(
                    title: "Highlight with note",
                    image: UIImage(systemName: "square.and.pencil")
                ) { [weak self] _ in
                    guard let self else { return }
                    onNote(quote, context)
                }
